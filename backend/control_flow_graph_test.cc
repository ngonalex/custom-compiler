...
/*TEST_F(ControlFlowGraphTest, IntegerExprIsVisited_0) {
...
TEST_F(ControlFlowGraphTest, AddExprIsVisited) {
  ...
  EXPECT_EQ(grapher_.GetOutput(), "t_0 <- 7\nt_1 <- 5\nt_2 <- t_0 + t_1\n");
}

TEST_F(ControlFlowGraphTest, SubtractExprIsVisited) {
...
TEST_F(ControlFlowGraphTest, MultiplyExprIsVisited) {
...
TEST_F(ControlFlowGraphTest, DivideExprIsVisited) {
...
TEST_F(ControlFlowGraphTest, NestedIntVisitationsWorkProperly) {
...
// ----- V2 Tests ------
TEST_F(ControlFlowGraphTest, SimpleAssignmentTest) {
...
TEST_F(ControlFlowGraphTest, DoubleIntAssignmentTest) {
  auto expr = make_unique<AssignmentFromArithExp>(
    make_unique<VariableExpr>("x"),
    make_unique<AddExpr>(
      make_unique<IntegerExpr>(5),
...
TEST_F(ControlFlowGraphTest, UnassignedVariable) {
...
TEST_F(ControlFlowGraphTest, VariabletoVariableAssignmentTest) {
...
// ----- V3 Tests -----
TEST_F(ControlFlowGraphTest, NestedLogicalsWithInts) {
...
TEST_F(ControlFlowGraphTest, ConditionalsWithTrueBranch) {
  Statement::Block statements;
...
TEST_F(ControlFlowGraphTest, ConditionalsWithFalseBranch) {
  Statement::Block statements;
...
TEST_F(ControlFlowGraphTest, SimpleLoop) {
...
TEST_F(ControlFlowGraphTest, EmptyLoop) {
...
TEST_F(ControlFlowGraphTest, InfinityLoop) {
...
TEST_F(ControlFlowGraphTest, LoopWithBody) {
  Statement::Block statements;
...
// ----- V4 Tests ------
TEST_F(ControlFlowGraphTest, FunctionDefTest) {
  auto foo_params = std::vector<std::unique_ptr<const VariableExpr>>();
  ...
  EXPECT_EQ(grapher_.GetOutput(), " <-  FUNCTIONDEF \nMkLabel func\n"
  " <-  FUNPROLOGUE \nt_0 <- 1\nt_1 <- 0\n"
  "t_2 <- t_0 + t_1\n <-  FUNEPILOGUE \n");
}

TEST_F(ControlFlowGraphTest, FunctionCallTest) {
  Statement::Block statements;
  ...
  EXPECT_EQ(grapher_.GetOutput(), "t_0 <- 10\nbob <- t_0\n"
    "t_1 <- bob VARLOAD \n <-  FUNCTIONCALL \n"
...
// ----- V5 Tests -----
TEST_F(ControlFlowGraphTest, SimpleTupleTest) {
  ...
  EXPECT_EQ(grapher_.GetOutput(), "bob <-  VARCHILDTUPLE \nt_0 <- 3\nbob <- t_0 NEWTUPLE \n");
}


TEST_F(ControlFlowGraphTest, AccessTupleTest) {
  auto ast = make_unique<AssignmentFromNewTuple>(make_unique<VariableExpr>("bob"), make_unique<IntegerExpr>(3));
...
TEST_F(ControlFlowGraphTest, TupleRHSDERTest) {
  auto ast = make_unique<AssignmentFromNewTuple>(make_unique<VariableExpr>("bob"), make_unique<IntegerExpr>(3));
...
TEST_F(ControlFlowGraphTest, NestedTupleTest){
...
TEST_F(ControlFlowGraphTest, AddTupleValueTest) {
  auto ast = make_unique<AssignmentFromNewTuple>(make_unique<VariableExpr>("bob"), make_unique<IntegerExpr>(3));
