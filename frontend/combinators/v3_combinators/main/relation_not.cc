...
ParseStatus NotRelationParser::do_parse(std::string inputProgram,
  ...
  ParseStatus result = notRelationParserFinal.parse(inputProgram, endCharacter);
    if (result.firstParsedCharacters == "!"){
        result.ast = make_node(unique_cast<const RelationalExpr>(std::move(result.second_ast)));
    }
  return result;
}

std::unique_ptr<const LogicalNotExpr> NotRelationParser::make_node(std::unique_ptr<const RelationalExpr> first_ae) {
