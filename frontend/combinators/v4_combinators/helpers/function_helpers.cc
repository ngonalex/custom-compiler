...
ParseStatus CommaOp::do_parse(std::string inputProgram,
                                      int startCharacter) {
...
ParseStatus FunctionVariableParsers::do_parse(std::string inputProgram, int startCharacter){
	int endCharacter = startCharacter;
	endCharacter += trim(inputProgram);
	...
	ParseStatus lastValue = secondAnd.do_parse(intermediateValue.remainingCharacters, intermediateValue.endCharacter);
