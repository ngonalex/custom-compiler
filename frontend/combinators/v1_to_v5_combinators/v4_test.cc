...
TEST(FunctionDeclParser, successFunctionDec1) {
  ...
  EXPECT_EQ(result.remainingCharacters, "");
  EXPECT_EQ(result.parsedCharacters,
            "funcvictor(prabal:Integer,Jason:String)->String{if(x==2){e=2;}"
            "else{f=3;}}");
  EXPECT_EQ(result.errorType, "");
...
TEST(FunctionDeclParser, successFunctionDec2) {
  FunctionDeclParser parser;
  ParseStatus result = parser.do_parse(
      "func victor() -> String {if(x == 2){e = 2;} else {f = 3;}}", 0);
  ...
  EXPECT_EQ(result.remainingCharacters, "");
  EXPECT_EQ(result.parsedCharacters,
            "funcvictor()->String{if(x==2){e=2;}else{f=3;}}");
  EXPECT_EQ(result.errorType, "");
...
TEST(FunctionDeclParser, failFunctionDec1) {
  FunctionDeclParser parser;
  ParseStatus result =
      parser.do_parse("func victor(prabal : Integer) {x + 2}", 0);
...
TEST(FunctionDeclParser, successFunctionCall1) {
  ...
  ParseStatus result =
      parser.do_parse("return_val = victor(2+3*(2)+victor, jason)", 0);
