...
// Statement -> Assignment | Cond | Loop (should be fine given that the three
// are fine)

// Cond -> if Logic Block Block
// Loop -> while Logic Block

// Logic - > Rel and Rel | Rel or Rel | not Rel | Rel --> NOT is tricky for
// parsing.... Rel -> Expr < Expr | Expr <= Expr | Expr > Expr | Expr >= Expr |
// Expr == Expr (these should all terminate)
...
class Parser {
 public:
  ...
  Token::Type DoubleNext() { return program_.rbegin()[1].type(); }
  ...
  // AST Expressions
  std::unique_ptr<const ArithmeticExpr> MakeArithmeticExpr(
